...
{
    public partial class WEBCaja : Form
    {
        ...
        decimal totalEfectivoVentaEnCaja = 0,
                ...
                AbonosAMisVentasACredito = 0,
                AbonosAMisVentasACreditoDeOtrosUsuarios = 0,
                AbonosAOtrasVentasACreditoDeUsuarios = 0;
        ...
        {
            int auxiliarIDEmpleado = FormPrincipal.id_empleado;
            ...
            datosWeb.Columns.Add("ventasTarjeta");
            datosWeb.Columns.Add("ventasVale");
            datosWeb.Columns.Add("ventasCheque");
            datosWeb.Columns.Add("ventrasTransferencia");
            datosWeb.Columns.Add("ventasCredito");
            datosWeb.Columns.Add("ventasAbono");
            datosWeb.Columns.Add("ventasAnticipo");
            datosWeb.Columns.Add("ventasTotal");
            datosWeb.Columns.Add("anticipoEfectivo");
            datosWeb.Columns.Add("anticipoTarjeta");
            datosWeb.Columns.Add("anticipoVale");
            datosWeb.Columns.Add("anticipoCheque");
            datosWeb.Columns.Add("anticipoTransferencia");
            datosWeb.Columns.Add("anticipoTotal");
            datosWeb.Columns.Add("agregadoEfectivo");
            datosWeb.Columns.Add("agregadoTarjeta");
            datosWeb.Columns.Add("agregadoVale");
            datosWeb.Columns.Add("agregadoCheque");
            datosWeb.Columns.Add("agregadoTransferencia");
            datosWeb.Columns.Add("agregadoTotal");
            datosWeb.Columns.Add("retiradoEfectivo");
            datosWeb.Columns.Add("retiradoTarjeta");
            datosWeb.Columns.Add("retiradoVale");
            datosWeb.Columns.Add("retiradoCheque");
            datosWeb.Columns.Add("retiradoTransferencia");
            datosWeb.Columns.Add("retiradoDevolucion");
            datosWeb.Columns.Add("retiradoTotal");
            datosWeb.Columns.Add("cajaEfectivo");
            datosWeb.Columns.Add("cajaTarjeta");
            datosWeb.Columns.Add("cajaVale");
            datosWeb.Columns.Add("cajaCheque");
            datosWeb.Columns.Add("cajaTransferencia");
            datosWeb.Columns.Add("cajaTotal");
        ...
        {
            totalAbonoEfectivo = totalAbonoTarjeta = totalAbonoVales = totalAbonoCheque = totalAbonoTransferencia = totalAbonoRealizado = totalAbonoRealizadoDeOtrosUsuarios = totalAbonoRealizadoOtrasVentas = AbonosAMisVentasACredito = AbonosAMisVentasACreditoDeOtrosUsuarios = AbonosAOtrasVentasACreditoDeUsuarios = 0;
        }
        ...
        private void generarReporteHojaCarta()
        {
            ...
            var fechaCorteCaja = string.Empty;
            #endregion
            ...
            var consultaIdUltimoCorteCaja = string.Empty;
            var idCorteCaja = 0;
            if (!FormPrincipal.userNickName.Contains("@"))
            {
                consultaIdUltimoCorteCaja = $"Select ID from caja where IDUsuario = '{FormPrincipal.userID}' and Operacion = 'corte' order by ID desc limit 1";
                using (DataTable dtIdCorteCajaRealizado = cn.CargarDatos(consultaIdUltimoCorteCaja))
                {
                    if (!dtIdCorteCajaRealizado.Rows.Count.Equals(0))
                    {
                        DataRow drIdCorteCajaRealizado = dtIdCorteCajaRealizado.Rows[0];
                        idCorteCaja = Convert.ToInt32(drIdCorteCajaRealizado["ID"].ToString());
                    }
                }
                using (DataTable dtSaldoInicial = cn.CargarDatos(cs.cargarSaldoInicialAdministrador()))
                {
            ...
            {
                consultaIdUltimoCorteCaja = $"Select ID from caja where IDUsuario = '{FormPrincipal.userID}' and IdEmpleado = '{FormPrincipal.id_empleado}' and Operacion = 'corte' order by ID desc limit 1";
                using (DataTable dtIdCorteCajaRealizado = cn.CargarDatos(consultaIdUltimoCorteCaja))
                {
                    if (!dtIdCorteCajaRealizado.Rows.Count.Equals(0))
                    {
            ...
            using (DataTable dtUsuario = cn.CargarDatos($"SELECT Usuario, RazonSocial FROM usuarios WHERE ID = '{FormPrincipal.userID}'"))
            {
                if (!dtUsuario.Rows.Count.Equals(0))
                {
                    DataRow drRazonSocialUsuario = dtUsuario.Rows[0];
                    nombreUsuario = drRazonSocialUsuario["RazonSocial"].ToString();
                    nombreEmpleado = drRazonSocialUsuario["Usuario"].ToString();
                }
            }
            #endregion
            ...
            if (FormPrincipal.id_empleado > 0)
            {
                using (DataTable dtEmpleado = cn.CargarDatos($"SELECT usuario FROM empleados WHERE ID = '{FormPrincipal.id_empleado}'"))
                {
                    if (!dtEmpleado.Rows.Count.Equals(0))
                    {
                        DataRow drEmpleado = dtEmpleado.Rows[0];
                        nombreEmpleado = drEmpleado["usuario"].ToString();
                    }
                }
            }
            #endregion
            ...
            using (DataTable dtCorteDeCaja = cn.CargarDatos($"SELECT FechaOperacion FROM caja WHERE ID = '{idUltimoCorteDeCaja}'"))
            {
                if (!dtCorteDeCaja.Rows.Count.Equals(0))
                {
                    DataRow drFechaCorteCaja = dtCorteDeCaja.Rows[0];
                    fechaCorteCaja = drFechaCorteCaja["FechaOperacion"].ToString();
                }
            }
            #endregion
                    ...
                    if (!dtSeccionVentaAdministrador.Rows.Count.Equals(0))
                    {
                        foreach (DataRow item in dtSeccionVentaAdministrador.Rows)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(item["Credito"].ToString()))
                            {
                                ...
                                if (cantidadCreditoResultadoBaseDeDatos > 0)
                                {
                                    cantidadCredito = cantidadCreditoResultadoBaseDeDatos;
                                }
                                else if (cantidadCreditoResultadoBaseDeDatos.Equals(0))
                                {
                                    cantidadCredito = 0;
