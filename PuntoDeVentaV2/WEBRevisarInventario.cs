...
{
    public partial class WEBRevisarInventario : Form
    {
        ...
        private void RevisarInventario_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                con.EjecutarConsulta($"DELETE FROM sesioninventario WHERE IDEmpleado = '{idEmpleado}'");
                //Se cierra la sesion alv
            }
            catch (Exception)
            {
                Thread.Sleep(6000);
                ...
                using (DataTable dt = cn2.CargarDatos($"SELECT * FROM peticiones WHERE Cliente = '{FormPrincipal.userNickName.Split('@')[0]}' AND Empleado = '{idEmpleado}'"))
                {
                    ...
                    //if (true)
                    {
                        foreach (DataRow peticion in dt.Rows)
                        {
                            ...
                            {
                                case "ProductoID":
                                    string id = peticion["Tipo"].ToString();
        ...
        {
            txtBoxBuscarCodigoBarras.Text = id;
            busquedaNormal = true;
            buscarCodigoBarras();
            busquedaNormal = false;
            using (var productos = cn.CargarDatos(cs.BuscarIDPreductoPorCodigoDeBarras(txtBoxBuscarCodigoBarras.Text)))
        ...
        private void verificadorSesion_Tick(object sender, EventArgs e)
        {
            using (DataTable dt = con.CargarDatos($"SELECT * FROM sesioninventario WHERE IDEmpleado = '{idEmpleado}'"))
            {
