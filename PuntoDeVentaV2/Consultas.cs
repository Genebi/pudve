...
namespace PuntoDeVentaV2
{
    class Consultas
    {
        ...
        public string GuardarHistorialPrecios(string[] datos)
       {
        ...
        public string VerComoDesdeOtroEmpleadoVentasPagadasPorFechasYBusqueda(int status, string FechaInicial, string FechaFinal, string busqueda)
        {
        ...
        public string imprimirTicketPresupuesto(int idPresupuestoRealizado)
        {
            var consulta = $"SET lc_time_names = 'es_MX'; SELECT DISTINCT CONCAT( DATE_FORMAT( Vent.FechaOperacion, '%W - %e/%M/%Y' ), ' ', TIME_FORMAT( Vent.FechaOperacion, '%h:%i:%s %p' ) ) AS 'FechaDeCompra', CONCAT( 'Folio: ', Vent.Folio ) AS 'FolioTicket', IF ( Usr.LogoTipo = '' OR Usr.LogoTipo IS NULL, '', Usr.LogoTipo ) AS 'LogoTipo', IF ( Usr.RazonSocial = '' OR Usr.RazonSocial IS NULL, '', Usr.RazonSocial ) AS 'RazonSocial', IF ( Usr.nombre_comercial = '' OR Usr.nombre_comercial IS NULL, '', CONCAT( 'NOMBRE COMERCIAL: ', Usr.nombre_comercial ) ) AS 'NombreComercial', CONCAT( IF ( Usr.Calle = '' OR Usr.Calle IS NULL, '', CONCAT( 'DIRECCION: ', Usr.Calle ) ), IF ( Usr.NoExterior = '' OR Usr.NoExterior IS NULL, '', CONCAT( ' #', Usr.NoExterior ) ), IF ( Usr.NoInterior = '' OR Usr.NoInterior IS NULL, '', CONCAT( ', INTERIOR: ', Usr.NoInterior ) ), IF ( Usr.Municipio = '' OR Usr.Municipio IS NULL, '', CONCAT( ', ', Usr.Municipio ) ), IF ( Usr.Estado = '' OR Usr.Estado IS NULL, '', CONCAT( ', ', Usr.Estado ) ) ) AS 'Direccion1', CONCAT( IF ( Usr.Colonia = '' OR Usr.Colonia IS NULL, '', CONCAT( 'COLONIA: ', Usr.Colonia ) ), IF ( Usr.CodigoPostal = '' OR Usr.CodigoPostal IS NULL, '', CONCAT( ', C.P.:', Usr.CodigoPostal ) ) ) AS 'Direccion2', IF ( Usr.RFC = '' OR Usr.RFC IS NULL, '', Usr.RFC ) AS 'RFC', IF ( Usr.Email = '' OR Usr.Email IS NULL, '', Usr.Email ) AS 'Correo', IF ( Usr.Telefono = '' OR Usr.Telefono IS NULL, '', Usr.Telefono ) AS 'Telefono', IF ( Clte.RazonSocial = '' OR Clte.RazonSocial IS NULL, IF ( Vent.IDCliente = '0', 'PUBLICO GENERAL', '' ), Clte.RazonSocial ) AS 'ClienteNombre', IF ( Clte.RFC = '' OR Clte.RFC IS NULL, IF ( Vent.IDCliente = '0', 'RFC: XAXX010101000', '' ), CONCAT( 'RFC: ', Clte.RFC ) ) AS 'ClienteRFC', CONCAT( IF ( Clte.Calle = '' OR Clte.Calle IS NULL, '', CONCAT( 'DOMICILIO: CALLE/AV.: ', Clte.Calle ) ), IF ( Clte.NoExterior = '' OR Clte.NoExterior IS NULL, '', CONCAT( ' #', Clte.NoExterior ) ), IF ( Clte.NoInterior = '' OR Clte.NoInterior IS NULL, '', CONCAT( ', INTERIOR: ', Clte.NoInterior ) ), IF ( Clte.Localidad = '' OR Clte.Localidad IS NULL, '', CONCAT( ', LOCALIDAD: ', Clte.Localidad ) ), IF ( Clte.Municipio = '' OR Clte.Municipio IS NULL, '', CONCAT( ', MUNICIPIO: ', Clte.Municipio ) ) ) AS 'ClienteDomicilio', CONCAT( IF ( Clte.Colonia = '' OR Clte.Colonia IS NULL, '', CONCAT( 'COLONIA: ', Clte.Colonia ) ), IF ( Clte.CodigoPostal = '' OR Clte.CodigoPostal IS NULL, '', CONCAT( ', C.P.: ', Clte.CodigoPostal ) ) ) AS 'ClienteColoniaCodigoPostal', IF ( Clte.Email = '' OR Clte.Email IS NULL, '', CONCAT( 'CORREO: ', Clte.Email ) ) AS 'ClienteCorreo', IF ( Clte.Telefono = '' OR Clte.Telefono IS NULL, '', CONCAT( 'TELEFONO: ', Clte.Telefono ) ) AS 'ClienteTelefono', IF ( Vent.FormaPago = '' OR Vent.FormaPago IS NULL, '', CONCAT( 'FORMA DE PAGO: ', UPPER( Vent.FormaPago ) ) ) AS 'FormaDePago', IF ( DetVent.Referencia = '' OR DetVent.Referencia IS NULL, '', CONCAT( 'REFERENCIA: ', UPPER( DetVent.Referencia ) ) ) AS 'Referencia', ProdVent.Cantidad AS 'ProductoCantidad', ProdVent.Nombre AS 'ProductoNombre', CONCAT( '$ ', FORMAT( ProdVent.Precio, 2 ) ) AS 'ProductoPrecio', CONCAT( '$ ', FORMAT( ProdVent.descuento, 2 ) ) AS 'ProductoDescuento', IF ( ( ( ProdVent.Precio * ProdVent.Cantidad ) - ProdVent.descuento ) IS NULL, CONCAT( '$', FORMAT( 0, 2 ) ), CONCAT( '$ ', FORMAT( ( ( ProdVent.Precio * ProdVent.Cantidad ) - ProdVent.descuento ), 2 ) ) ) AS 'ProductoImporte', CONCAT( 'Descuento productos: ', IF ( Vent.Descuento IS NULL, CONCAT( '$ ', FORMAT( 0, 2 ) ), CONCAT( '$ ', FORMAT( Vent.Descuento, 2 ) ) ) ) AS 'DescuentosDeProductos', CONCAT( 'TOTAL: ', IF ( Vent.Total IS NULL, CONCAT( '$ ', FORMAT( 0, 2 ), ' ' ), CONCAT( '$ ', FORMAT( Vent.Total, 2 ), ' ' ) ) ) AS 'TotalGeneral', IF ( editarticket.MensajeTicket IS NULL OR editarticket.MensajeTicket = '.' OR editarticket.MensajeTicket = '', '', editarticket.MensajeTicket ) AS 'MensajeDelTicket', IF ( Vent.Folio = '' OR Vent.Folio IS NULL, '', Vent.Folio ) AS 'CodigoBarrasTicketVenta' FROM productosventa AS ProdVent INNER JOIN ventas AS Vent ON ( ProdVent.IDVenta = Vent.ID ) INNER JOIN detallesventa AS DetVent ON ( DetVent.IDVenta = Vent.ID ) INNER JOIN usuarios AS Usr ON ( Usr.ID = Vent.IDUsuario ) INNER JOIN configuracion AS Config ON ( Config.IDUsuario = Usr.ID ) LEFT JOIN clientes AS Clte ON ( Clte.ID = Vent.IDCliente ) LEFT JOIN editarticket ON ( editarticket.IDUsuario = Usr.ID ) WHERE Vent.IDUsuario = '{FormPrincipal.userID}' AND Vent.`Status` = '2' AND ProdVent.IDVenta = '{idPresupuestoRealizado}'";
        ...
        {
            var consulta = $"SELECT CONCAT( '$ ', IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) ) AS 'Efectivo', CONCAT( '$ ', IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) ) AS 'Tarjeta', CONCAT( '$ ', IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) ) AS 'Vales', CONCAT( '$ ', IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) ) AS 'Cheque', CONCAT( '$ ', IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) ) AS 'Transferencia', CONCAT( '$ ', IF ( ( SUM( Efectivo ) + SUM( Tarjeta ) + SUM( Vales ) + SUM( Cheque ) + SUM( Transferencia ) ) = '' OR ( SUM( Efectivo ) + SUM( Tarjeta ) + SUM( Vales ) + SUM( Cheque ) + SUM( Transferencia ) ) IS NULL, FORMAT( '0', 2 ), FORMAT( ( SUM( Efectivo ) + SUM( Tarjeta ) + SUM( Vales ) + SUM( Cheque ) + SUM( Transferencia ) ), 2 ) ) ) AS 'TotalAnticipos' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo'";
        ...
        {
            var consulta = $"SELECT CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Efectivo ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialEfectivo, 2 ) ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Efectivo', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Tarjeta ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTarjeta, 2 ) ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Tarjeta', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Vales ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialVales, 2 ) ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Vales', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Cheque ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialCheque, 2 ) ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Cheque', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Transferencia ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTransferencia, 2 ) ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Transferencia', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT SUM( Efectivo ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialEfectivo, 2 ) ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Tarjeta ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTarjeta, 2 ) ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Vales ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialVales, 2 ) ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Cheque ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialCheque, 2 ) ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Transferencia ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTransferencia, 2 ) ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Total', CONCAT( '$ ', FORMAT( ( SELECT IF ( CantidadRetiradaDelCorte = '' OR CantidadRetiradaDelCorte IS NULL, FORMAT( '0', 2 ), CantidadRetiradaDelCorte ) AS 'Retiro' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaInicio}' ), 2 ) ) AS 'Retiro', CONCAT( '$ ', FORMAT( ( ( ( ( ( CONVERT ( ( SELECT SUM( Efectivo ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Efectivo ), 2 ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialEfectivo, 2 ) ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Tarjeta ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Tarjeta ), 2 ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTarjeta, 2 ) ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Vales ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Vales ), 2 ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialVales, 2 ) ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Cheque ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Cheque ), 2 ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialCheque, 2 ) ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT SUM( Transferencia ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SUM( Transferencia ), 2 ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IdEmpleado = '0' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, FORMAT( '0', 2 ), FORMAT( SaldoInicialTransferencia, 2 ) ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaFin}' ) ) ) - ( SELECT IF ( CantidadRetiradaDelCorte = '' OR CantidadRetiradaDelCorte IS NULL, FORMAT( '0', 2 ), CantidadRetiradaDelCorte ) AS 'Retiro' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND IDCorteDeCaja = '{IDCajaInicio}' ), 2 ) ) AS 'Resto'";
        ...
        {
            var consulta = $"SELECT CONCAT( '$ ', FORMAT( IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ), 2 ) ) AS 'Efectivo', CONCAT( '$ ', FORMAT( IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ), 2 ) ) AS 'Tarjeta', CONCAT( '$ ', FORMAT( IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ), 2 ) ) AS 'Vales', CONCAT( '$ ', FORMAT( IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ), 2 ) ) AS 'Cheque', CONCAT( '$ ', FORMAT( IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ), 2 ) ) AS 'Transferencia', CONCAT( '$ ', FORMAT( ( ( IF ( Credito = '' OR Credito IS NULL, '0', SUM( Credito ) ) ) - ( SELECT IF ( Abono.Total = '' OR Abono.Total IS NULL, '0', SUM( Abono.Total ) ) FROM abonos AS Abono WHERE Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '{FormPrincipal.id_empleado}' AND Abono.FechaOperacion >= '{fechaLimiteInferior}' AND Abono.FechaOperacion <= '{fechaLimiteSuperior}' ) ), 2 ) ) AS 'Credito', ( CONCAT( '$ ', FORMAT( CONVERT ( ( SELECT IF ( Abono.Total = '' OR Abono.Total IS NULL, '0', SUM( Abono.Total ) ) FROM abonos AS Abono WHERE Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '{FormPrincipal.id_empleado}' AND Abono.FechaOperacion >= '{fechaLimiteInferior}' AND Abono.FechaOperacion <= '{fechaLimiteSuperior}' ), DECIMAL ), 2 ) ) ) AS 'Abonos', CONCAT( '$ ', FORMAT( IF ( Anticipo = '' OR Anticipo IS NULL, '0', SUM( Anticipo ) ), 2 ) ) AS 'Anticipo', CONCAT( '$ ', FORMAT( ( IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) + IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) + IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) + IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) + IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) + ( ( IF ( Credito = '' OR Credito IS NULL, '0', SUM( Credito ) ) - ( SELECT IF ( Abono.Total = '' OR Abono.Total IS NULL, '0', SUM( Abono.Total ) ) FROM abonos AS Abono WHERE Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '{FormPrincipal.id_empleado}' AND Abono.FechaOperacion >= '{fechaLimiteInferior}' AND Abono.FechaOperacion <= '{fechaLimiteSuperior}' ) ) + ( SELECT IF ( Abono.Total = '' OR Abono.Total IS NULL, '0', SUM( Abono.Total ) ) FROM abonos AS Abono WHERE Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '{FormPrincipal.id_empleado}' AND Abono.FechaOperacion >= '{fechaLimiteInferior}' AND Abono.FechaOperacion <= '{fechaLimiteSuperior}' ) ) + IF ( Anticipo = '' OR Anticipo IS NULL, '0', SUM( Anticipo ) ) ), 2 ) ) AS 'TotalVentas' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta'";
        ...
        {
            var consulta = $"SELECT CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, '0', SaldoInicialEfectivo ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ), 2 ) ) AS 'Efectivo', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, '0', SaldoInicialTarjeta ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Tarjeta', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, '0', SaldoInicialVales ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Vales', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, '0', SaldoInicialCheque ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Cheque', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, '0', SaldoInicialTransferencia ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ), 2 ) ) AS 'Transferencia', CONCAT( '$ ', FORMAT( ( ( ( ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, '0', SaldoInicialEfectivo ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, '0', SaldoInicialTarjeta ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, '0', SaldoInicialVales ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, '0', SaldoInicialCheque ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, '0', SaldoInicialTransferencia ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ), 2 ) ) AS 'Total', CONCAT( '$ ', FORMAT( ( SELECT IF ( CantidadRetiradaDelCorte = '' OR CantidadRetiradaDelCorte IS NULL, '0', CantidadRetiradaDelCorte ) AS 'Retiro' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ), 2 ) ) AS 'Retiro', CONCAT( '$ ', FORMAT( ( ( ( ( ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'anticipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Efectivo = '' OR Efectivo IS NULL, '0', SUM( Efectivo ) ) AS 'Efectivo' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialEfectivo = '' OR SaldoInicialEfectivo IS NULL, '0', SaldoInicialEfectivo ) AS 'Efectivo' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Tarjeta = '' OR Tarjeta IS NULL, '0', SUM( Tarjeta ) ) AS 'Tarjeta' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTarjeta = '' OR SaldoInicialTarjeta IS NULL, '0', SaldoInicialTarjeta ) AS 'Tarjeta' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaFin}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Vales = '' OR Vales IS NULL, '0', SUM( Vales ) ) AS 'Vales' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialVales = '' OR SaldoInicialVales IS NULL, '0', SaldoInicialVales ) AS 'Vales' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Cheque = '' OR Cheque IS NULL, '0', SUM( Cheque ) ) AS 'Cheque' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialCheque = '' OR SaldoInicialCheque IS NULL, '0', SaldoInicialCheque ) AS 'Cheque' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) + ( ( ( ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'venta' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'antipo' ), DECIMAL ) ) + ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'deposito' ), DECIMAL ) ) ) - ( CONVERT ( ( SELECT IF ( Transferencia = '' OR Transferencia IS NULL, '0', SUM( Transferencia ) ) AS 'Transferencia' FROM caja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND ID <= '{IDCajaInicio}' AND ID >= '{IDCajaFin}' AND Operacion = 'retiro' ), DECIMAL ) ) ) + ( SELECT IF ( SaldoInicialTransferencia = '' OR SaldoInicialTransferencia IS NULL, '0', SaldoInicialTransferencia ) AS 'Transferencia' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ) ) ) - ( SELECT IF ( CantidadRetiradaDelCorte = '' OR CantidadRetiradaDelCorte IS NULL, '0', CantidadRetiradaDelCorte ) AS 'Retiro' FROM historialcortesdecaja WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '{FormPrincipal.id_empleado}' AND IDCorteDeCaja = '{IDCajaInicio}' ), 2 ) ) AS 'Resto'";
