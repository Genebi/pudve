...
namespace PuntoDeVentaV2
{
    class Consultas
    {
        ...
        public string GuardarHistorialPrecios(string[] datos)
       {
        ...
        {
            var consulta = $"SELECT Usr.RazonSocial, Usr.RFC, Usr.Regimen, CONCAT( IF ( Usr.Calle = '' OR Usr.Calle IS NULL, '', Usr.Calle ), IF ( Usr.NoExterior = '' OR Usr.NoExterior IS NULL, '', CONCAT( ' #', Usr.NoExterior ) ), IF ( Usr.NoInterior = '' OR Usr.NoInterior IS NULL, '', CONCAT( ', INTERIOR: ', Usr.NoInterior ) ), IF ( Usr.Municipio = '' OR Usr.Municipio IS NULL, '', CONCAT( ', ', Usr.Municipio ) ), IF ( Usr.Estado = '' OR Usr.Estado IS NULL, '', CONCAT( ', ', Usr.Estado ) ) ) AS 'Domicilio', IF ( Usr.Colonia = '' OR Usr.Colonia IS NULL, '', Usr.Colonia ) AS 'Colonia', Usr.Email AS 'Correo', Usr.Telefono AS 'telefono', CONCAT( 'Folio: ', vent.Folio, ' Serie: ', vent.Serie ) AS 'folio', vent.FechaOperacion AS 'fecha', vent.FormaPago AS 'FormadePago', PV.Cantidad AS 'Cantidad', PV.Nombre AS 'Nombre', CONCAT( '$', FORMAT( PV.Precio, 2 ) ) AS 'precioUnidad', CONCAT( '$', FORMAT( PV.Precio * PV.Cantidad, 2 ) ) AS 'importe', SUBSTRING_INDEX( PV.descuento, '-', 1 ) AS 'DescuentoDirectoProducto', SUBSTRING_INDEX( PV.descuento, '-', - 1 ) AS 'DescuentoPorCiento', PV.Precio - PV.descuento AS 'P/UConDescuento', vent.Descuento + vent.Total AS 'SubTotal', vent.Descuento AS 'Descuento', vent.Total AS 'total', CONCAT( ' Debo y pagaré a la orden ', Usr.RazonSocial, ' en esta ciudad ____________________________________ antes del dia ', vent.FechaOperacion, ', la cantidad de ', vent.Total, ' valor de la mercancía recibida a mi entera satisfacción. Este pagaré es mercantil y está regido por la Ley General de Títulos y operaciones de Crédito en su artículo 173 y correlativo por no ser pagaré domiciliado. Si este documento no es pagado en su vencimiento causará intereses moratorios del _______% mensual. Acepto(mos): _________________________________________________' ) AS 'Mensaje', IF ( cli.RazonSocial = '' OR cli.RazonSocial IS NULL, 'PUBLICO GENERAL', cli.RazonSocial ) AS 'NombreCliente', IF ( cli.RFC = '' OR cli.RFC IS NULL, 'XAXX010101000', cli.RFC ) AS 'RFCCliente', CONCAT ( IF ( cli.Calle = '' OR cli.Calle IS NULL, '', cli.Calle ), IF ( cli.NoExterior = '' OR cli.NoExterior IS NULL, '', CONCAT( ' #', cli.NoExterior ) ), IF ( cli.NoInterior = '' OR cli.NoInterior IS NULL, '', CONCAT( ', INTERIOR: ', cli.NoInterior ) ), IF ( cli.Municipio = '' OR cli.Municipio IS NULL, '', CONCAT( ', ', cli.Municipio ) ), IF ( cli.Estado = '' OR cli.Estado IS NULL, '', CONCAT( ', ', cli.Estado ) ) ) AS 'DomicilioCliente', cli.Pais AS 'Pais', cli.Localidad AS 'Localidad', cli.CodigoPostal AS 'CP', cli.Colonia AS 'ColoniaCliente', cli.Email AS 'CorreoCliente', cli.Telefono AS 'telefonoCliente' FROM ventas AS vent INNER JOIN usuarios AS Usr ON ( usr.ID = vent.IDUsuario ) INNER JOIN productosventa AS PV ON ( PV.IDVenta = vent.ID ) INNER JOIN clientes AS cli ON ( cli.ID = vent.IDCliente ) WHERE usr.ID = '{FormPrincipal.userID}' AND vent.ID = '{IDVenta}'";
            return consulta;
        }
