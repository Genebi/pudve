...
namespace PuntoDeVentaV2
{
    class Consultas
    {
        ...
        {
            var consulta = $"SELECT Vent.*, Usr.Usuario, IF ( Clte.RazonSocial IS NULL, 'PUBLICO GENERAL', Clte.RazonSocial ) AS 'Consumidor', IF ( Emp.nombre IS NULL, CONCAT( Usr.Usuario, ' (ADMIN)' ), CONCAT( Emp.nombre, ' (EMPLEADO)' ) ) AS 'Vendedor', Abono.Efectivo AS 'Efectivo', Abono.Tarjeta AS 'Tarjeta', Abono.Vales AS 'Vales', Abono.Cheque AS 'Cheque', Abono.Transferencia AS 'Transferencia', ( Vent.Total - ( Abono.Efectivo + Abono.Tarjeta + Abono.Vales + Abono.Cheque + Abono.Transferencia ) ) AS 'Pendiente de pago' FROM ventas AS Vent INNER JOIN usuarios AS Usr ON ( Usr.ID = Vent.IDUsuario ) LEFT JOIN clientes AS Clte ON ( Clte.ID = Vent.IDCliente ) LEFT JOIN empleados AS Emp ON ( Emp.ID = Vent.IDEmpleado ) INNER JOIN Abonos AS Abono ON ( Abono.IDVenta = Vent.ID ) WHERE Vent.`Status` = '{estado}' AND Vent.IDUsuario = '{FormPrincipal.userID}' AND Vent.FechaOperacion BETWEEN '{ultimaFechaDeCorte}.999999' AND '{rangoFechaLimite}.999999' ORDER BY Vent.ID DESC";
