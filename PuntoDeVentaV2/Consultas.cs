...
namespace PuntoDeVentaV2
{
    class Consultas
    {
        ...
        public string GuardarHistorialPrecios(string[] datos)
       {
        ...
        {
            var consulta = $"SELECT * FROM abonos AS Abono WHERE Abono.ID >= ( SELECT MAX(ID) AS 'ID' FROM abonos WHERE IDVenta NOT IN ( SELECT MAX(ID) AS 'ID' FROM ventas WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND `Status` = '4' AND FechaOperacion > '{fechaOperacion}' ) AND IDEmpleado = '0' LIMIT 1 ) AND Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '0' AND Abono.FechaOperacion > '{fechaOperacion}'";
