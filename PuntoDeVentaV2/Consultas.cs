...
namespace PuntoDeVentaV2
{
    class Consultas
    {
        ...
        public string GuardarHistorialPrecios(string[] datos)
       {
        ...
        {
            var consulta = $"( SELECT IF ( Abono.IDEmpleado = '0', 'Propia', 'Ajena' ) AS 'Propia', IF ( Abono.IDEmpleado = '' OR Abono.IDEmpleado IS NULL, '0', Abono.IDEmpleado ) AS 'IDEmpleado', FORMAT( IF ( Abono.Efectivo = '' OR Abono.Efectivo IS NULL, '0', Abono.Efectivo ), 2 ) AS 'Efectivo', FORMAT( IF ( Abono.Tarjeta = '' OR Abono.Tarjeta IS NULL, '0', Abono.Tarjeta ), 2 ) AS 'Tarjeta', FORMAT( IF ( Abono.Vales = '' OR Abono.Vales IS NULL, '0', Abono.Vales ), 2 ) AS 'Vales', FORMAT( IF ( Abono.Cheque = '' OR Abono.Cheque IS NULL, '0', Abono.Cheque ), 2 ) AS 'Cheque', FORMAT( IF ( Abono.Transferencia = '' OR Abono.Transferencia IS NULL, '0', Abono.Transferencia ), 2 ) AS 'Transferencia', FORMAT( ( ( IF ( Abono.Efectivo = '' OR Abono.Efectivo IS NULL, '0', Abono.Efectivo ) ) + ( IF ( Abono.Tarjeta = '' OR Abono.Tarjeta IS NULL, '0', Abono.Tarjeta ) ) + ( IF ( Abono.Vales = '' OR Abono.Vales IS NULL, '0', Abono.Vales ) ) + ( IF ( Abono.Cheque = '' OR Abono.Cheque IS NULL, '0', Abono.Cheque ) ) + ( IF ( Abono.Transferencia = '' OR Abono.Transferencia IS NULL, '0', Abono.Transferencia ) ) ), 2 ) AS 'Total' FROM ventas AS Vent INNER JOIN abonos AS Abono ON ( Abono.IDVenta = Vent.ID ) WHERE Vent.IDUsuario = '{idUsuario}' AND Vent.IDEmpleado = '0' AND Vent.FormaPago = 'Crédito' AND Vent.FechaOperacion > '{ultimaFechaDeCorte}' ) UNION ( SELECT IF ( Emp.nombre = '' OR Emp.nombre IS NULL, 'Ajena', 'Propia' ) AS 'Propia', IF ( Abono.IDEmpleado = '' OR Abono.IDEmpleado IS NULL, '0', Abono.IDEmpleado ) AS 'IDEmpleado', FORMAT( IF ( Abono.Efectivo = '' OR Abono.Efectivo IS NULL, '0', Abono.Efectivo ), 2 ) AS 'Efectivo', FORMAT( IF ( Abono.Tarjeta = '' OR Abono.Tarjeta IS NULL, '0', Abono.Tarjeta ), 2 ) AS 'Tarjeta', FORMAT( IF ( Abono.Vales = '' OR Abono.Vales IS NULL, '0', Abono.Vales ), 2 ) AS 'Vales', FORMAT( IF ( Abono.Cheque = '' OR Abono.Cheque IS NULL, '0', Abono.Cheque ), 2 ) AS 'Cheque', FORMAT( IF ( Abono.Transferencia = '' OR Abono.Transferencia IS NULL, '0', Abono.Transferencia ), 2 ) AS 'Transferencia', FORMAT( ( ( IF ( Abono.Efectivo = '' OR Abono.Efectivo IS NULL, '0', Abono.Efectivo ) ) + ( IF ( Abono.Tarjeta = '' OR Abono.Tarjeta IS NULL, '0', Abono.Tarjeta ) ) + ( IF ( Abono.Vales = '' OR Abono.Vales IS NULL, '0', Abono.Vales ) ) + ( IF ( Abono.Cheque = '' OR Abono.Cheque IS NULL, '0', Abono.Cheque ) ) + ( IF ( Abono.Transferencia = '' OR Abono.Transferencia IS NULL, '0', Abono.Transferencia ) ) ), 2 ) AS 'Total' FROM ventas AS Vent INNER JOIN abonos AS Abono ON ( Abono.IDVenta = Vent.ID ) LEFT JOIN empleados AS Emp ON ( Emp.ID = Abono.IDEmpleado ) WHERE Vent.IDUsuario = '{idUsuario}' AND Vent.IDEmpleado != '0' AND Vent.FormaPago = 'Crédito' AND Vent.FechaOperacion > '{ultimaFechaDeCorte}' )";
        ...
        {
            var consulta = $"SELECT * FROM abonos AS Abono WHERE Abono.ID > ( SELECT ID FROM abonos WHERE IDVenta NOT IN ( SELECT ID FROM ventas WHERE IDUsuario = '{FormPrincipal.userID}' AND IDEmpleado = '0' AND `Status` = '4' AND FechaOperacion > '{fechaOperacion}' ) AND IDEmpleado = '0' LIMIT 1 ) AND Abono.IDUsuario = '{FormPrincipal.userID}' AND Abono.IDEmpleado = '0' AND Abono.FechaOperacion > '{fechaOperacion}'";
