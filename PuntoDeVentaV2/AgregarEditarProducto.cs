...
namespace PuntoDeVentaV2
{
    public partial class AgregarEditarProducto : Form
    {
		...
		****************************/
                                ...
                                if (contLblHijo.Name.Trim() == "lblNombrechk" + nuevoTextoBuscado)
                                {
                                    ...
                                    if (DatosSourceFinal == 2)
                                    {
                                        ...
                                        if (!resultadoBuscarDetalleGeneralProducto.Count().Equals(0))//En caso de ya estar registrado el detalle.
                                        {
                                            ...
                                            ActualizarDetallesDinamicos.Add($"UPDATE DetallesProductoGenerales SET IDDetalleGral = '{dataSave[3]}' WHERE IDProducto = '{dataSave[0]}' AND IDUsuario = '{FormPrincipal.userID}' AND panelContenido = '{dataSave[2]}'");
                                        }
                        ...
                        if (contSubHijo.Name == namePanel)
                        {
                            foreach (Control contLblHijo in contSubHijo.Controls.OfType<Control>())
                            {
                                ...
                                if (DatosSourceFinal == 2)
                                {
                                    ...
                                    {
                                        //int resultChangeProvaider = cn.EjecutarConsulta(cs.GuardarProveedorProducto(dataSave, 1));
                                        ActualizarDetallesDinamicos.Add($"UPDATE DetallesProducto SET Proveedor = '{dataSave[2]}', IDProveedor = '{dataSave[3]}' WHERE IDProducto = '{dataSave[0]}' AND IDUsuario = '{FormPrincipal.userID}'");

                                    }
                                    else if (resultadoBusquedaDetallesProducto.Count().Equals(0))
                                    {
                                        //int resultChangeProvaider = cn.EjecutarConsulta(cs.GuardarProveedorProducto(dataSave));
                                        InsertarDetallesDinamicos.Add($"INSERT INTO DetallesProducto (IDProducto, IDUsuario, Proveedor, IDProveedor)VALUES ('{dataSave[0]}', '{FormPrincipal.userID}', '{dataSave[2]}', '{dataSave[3]}')");
                                    }
                                }
                            }
                        }
                        ...
                        if (bValidNombreProducto && bValidPrecioProducto)
                        {
                            ...
                            if (bValidClaveInterna || bValidCodigoBarras)
                            {
                                ...
                                guardar = new string[] {
                                    ...
                                    txtPrecioCompra.Text, precioMayoreo, incluye_impuestos, nombre_cterceros, rfc_cterceros, cp_cterceros, regimen_cterceros, formatoDeVenta , esRenta};
                                ...
                                try
                                {
                                    ...
                                    cn.insertImage(cs.guardarImgProducto(""), img);
                        ...
                        if (bValidNombreProducto && bValidPrecioProducto)
                        {
                            ...
                            if (bValidClaveInterna || bValidCodigoBarras)
                            {
                                ...
                                guardar = new string[] {
                                    ...
                                    stockNecesario, "0", txtPrecioCompra.Text, precioMayoreo, incluye_impuestos, nombre_cterceros, rfc_cterceros, cp_cterceros, regimen_cterceros, "0", esRenta
                                };
                ...
                if (SearchProdResult.Rows.Count != 0 && resultadoSearchNoIdentificacion == 0)
                {
                    ...
                    ****************************/
                    ...
                    guardar = new string[] {
                        ...
                        incluye_impuestos, nombre_cterceros, rfc_cterceros, cp_cterceros, regimen_cterceros, "0", esRenta
                    };
        ...
        {
            if (!ActualizarDetallesDinamicos.Count.Equals(0))
            {
                foreach (var item in ActualizarDetallesDinamicos)
                {
                    cn.EjecutarConsulta(item);
                }
            }
            if (!InsertarDetallesDinamicos.Count.Equals(0))
            {
                foreach (var item in InsertarDetallesDinamicos)
                {
                    cn.EjecutarConsulta(item);
                }
            }
        ...
        private void ValidarEntradaDeTexto(object sender, EventArgs e)
        {
            ...
            {
                if (resultado.Contains("|") || resultado.Contains("+") || resultado.Contains("'") || resultado.Contains("*") || resultado.Contains("/"))
                {
        ...
        private void txtCantPaqServ_TextChanged(object sender, EventArgs e)
        {
            ValidarEntradaDeTextoNumeros(sender, e);
        }
                        ...
                        if (bValidNombreProducto && bValidPrecioProducto)
                        {
                            ...
                            if (bValidClaveInterna || bValidCodigoBarras)
                            {
                                ...
                                guardar = new string[] {
                                    ...
                                    tipoDescuento, idUsrNvo, logoTipo, ProdServPaq, baseProducto, ivaProducto, impuestoProducto,
                                    mg.RemoverCaracteres(nombre), mg.RemoverPreposiciones(nombre), stockNecesario, stockMinimo,
                                    txtPrecioCompra.Text, precioMayoreo, "0", esRenta };
                ...
                if (SearchProdResult.Rows.Count != 0 && resultadoSearchNoIdentificacion == 0)
                {
			        ...
			        ****************************/
                    ...
                    guardar = new string[] {
                        ...
                        codigoBNvoInsert, claveProducto, claveUnidadMedida, tipoDescuentoNvoInsert, idUsrNvoInsert,
                        logoTipo, tipoProdNvoInsert, baseProducto, ivaProducto, impuestoProducto, mg.RemoverCaracteres(nombreNvoInsert),
                        mg.RemoverPreposiciones(nombreNvoInsert), stockNecesario, stockMinimo, txtPrecioCompra.Text, precioMayoreo, "0", esRenta
                    };
        ...
        private void AgregarEditarProducto_Load(object sender, EventArgs e)
        {
            ...
            if (config.Count > 0)
            {
                ...
                checkRenta.Visible = Convert.ToBoolean(config[32]);
            }
        ...
        private void agregarCombo()
        {
            ...
            if (conSinClaveInterna.Equals(false))
            {
                if (DatosSourceFinal.Equals(1) || DatosSourceFinal.Equals(2))
                {
                    ...
                    tLPCombo.Controls.Add(label2, 0, 2);               // Código de Barras Label
                    tLPCombo.Controls.Add(lblCodigoBarras, 1, 2);      // Simbolo Informativo
                    tLPCombo.Controls.Add(lblCodBarExtra, 0, 4);       // Simbolo Informativo gregar código Barras extra
                    ...
                    tLPCombo.Controls.Add(txtCodigoBarras, 0, 3);      // Código de Barras TextBox
                    tLPCombo.Controls.Add(btnGenerarCB, 1, 3);         // Código de Barras Button
                    ...
                    tLPCombo.Controls.Add(panelContenedor, 0, 4);      // Contenedor de Código Barras extra Panel
            ...
            else if (conSinClaveInterna.Equals(true))
            {
                ...
                tLPCombo.Controls.Add(panelContenedor, 0, 4);      // Código de Barras TextBox
                tLPCombo.SetColumnSpan(panelContenedor, 2);
                tLPCombo.SetRowSpan(panelContenedor, 2);
        ...
        private void agregarServicio()
        {
            ...
            if (conSinClaveInterna.Equals(false))
            {
                if (DatosSourceFinal.Equals(1) || DatosSourceFinal.Equals(2))
                {
                    ...
                    tLPServicio.Controls.Add(label2, 0, 2);               // Código de Barras Label
                    tLPServicio.Controls.Add(lblCodigoBarras, 1, 2);      // Simbolo Informativo
                    tLPServicio.Controls.Add(lblCodBarExtra, 0, 4);
                    ...
                    tLPServicio.Controls.Add(txtCodigoBarras, 0, 3);      // Código de Barras TextBox
                    tLPServicio.Controls.Add(btnGenerarCB, 1, 3);         // Código de Barras Button
                    ...
                    tLPServicio.Controls.Add(panelContenedor, 0, 4);      // Código de Barras TextBox
                ...
                if (DatosSourceFinal.Equals(3))
                {
                    ...
                    tLPServicio.Controls.Add(lblCodigoBarras, 1, 2);      // Simbolo Informativo
                    tLPServicio.Controls.Add(lblCodBarExtra, 0, 4);       // Simbolo Informativo gregar código Barras extra
                    ...
                    tLPServicio.Controls.Add(panelContenedor, 0, 4);      // Contenedor de Código Barras extra Panel
                    tLPServicio.SetColumnSpan(panelContenedor, 2);
                    tLPServicio.SetRowSpan(panelContenedor, 2);
