    ...
    public partial class detallesIngresosRetirosDinero : Form
    {
        ...
        private void detallesIngresosRetirosDinero_Load(object sender, EventArgs e)
        {
            ...
            if (!ultimoCorteCaja.Rows.Count.Equals(0))
            {
                ...
                DatosDeposito = cn.CargarDatos($"SELECT * FROM caja WHERE IDUsuario = {FormPrincipal.userID} AND idEmpleado = '{FormPrincipal.id_empleado}' AND FechaOperacion >= '{formatoFecha}' AND Operacion = 'deposito' OR Operacion = 'deposito' AND IDUsuario = {FormPrincipal.userID} AND idEmpleado = '{FormPrincipal.id_empleado}' AND FechaOperacion >= '{formatoFecha}' AND Concepto = 'Insert primer saldo inicial'");
